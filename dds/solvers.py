...
def sdeint_ito_em_scan_ou(
    ...
    def euler_step(ytpas, t_):
        ...
        # schedule for alpha_k(t, t-1) (equivlane to lbd in my code)
        ...
        # State update
        y_naug = (
            y_pas_naug * alpha_k  # sqrt(1-alpha_k) in eq (18)
            + f_aug[:, :dim] * beta_k**2  # 2 * (1 - sqrt(1-alpha_k_)) in eq (18)
        ...
        u_sq = (
            y_pas[:, -1] + f_aug[:, -1] * beta_k**2
        )  # ANGUS I think this must be the r_k update from alg 1 - how do the coefficients match?
        ...
        if detach:
            ...
            v_dw = np.einsum("ij,ij->i", g_aug_det[:, dim:-1], noise[:, :dim]) * beta_k
        ...
        y = np.concatenate(
            (y_naug, u_dw[..., None], log_is_weight[..., None], u_sq[..., None]),
            axis=-1,
        )  # ANGUS this explains the 3 augmented dimensions
...
def odeint_em_scan_ou(
    ...
    def euler_step(ytpas, t_):
        ...
        y = np.concatenate(
            (y_naug, np.zeros((y0.shape[0], 1)), u_sq[..., None]), axis=-1
        )
...
def sdeint_udp_ito_em_scan_ou(
    ...
    def euler_step(ytpas, t_):
        ...
        q_prime = y_latent * np.sin(theta) * (np.sqrt(m) / sigma) + q_obs * np.cos(
            theta
        )
        ...
        q_new = rt_1_mnsalpha_k * (
            q_prime + 2.0 * m * lambda_k * np.sqrt(alpha_k) * f_
        ) + (noise[:, :dim] * np.sqrt(m * alpha_k))

        # Stoch int (detached STL term) update
        udw_new = (
            np.squeeze(udw_old)
            + np.einsum("ij,ij->i", f_, noise[:, :dim])
            * np.sqrt(m)
            * rt_1_mnsalpha_k
            * lambda_k
            * 2.0
...
def controlled_ais_sdeint_ito_em_scan(
    ...
    def euler_step(ytpas, t_):
        ...
        delta_y = (
            f_full[:, :dim] * delta_t + rt2 * np.sqrt(gamma) * np.sqrt(delta_t) * noise_
        )
        y = y_pas[:, :dim] + delta_y
        b_full = b(y, t_, args)

        #     print(f_full, b_full, g_full, delta_t)
        coef = 1.0 / (2.0 * (delta_t) * gamma * 2.0)
