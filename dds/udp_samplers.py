...
class AugmentedAbstractFollmerSDEUDP(hk.Module):
    ...
    def __call__(self, batch_size, is_training=True, dt=None, ode=False):
        ...
        return self.sample_aug_trajectory(batch_size, key, dt=dt, ode=ode)
...
class AugmentedOUDFollmerSDEUDP(AugmentedAbstractFollmerSDEUDP):
    ...
    def sample_aug_trajectory(self, batch_size, key, dt=0.05, ode=False, **_):
        ...
        # ou based sampler.
        integrator = (
            sdeint_udp_ito_em_scan_ou_logged if self.log else sdeint_udp_ito_em_scan_ou
        )
        ...
        out_param_trajectory = np.concatenate((latent_traj, girsanov_terms), axis=-1)
